...
async def director_task(chapter_seed: str, active_characters: Optional[list[str]] = None) -> str:
...
def tactician_task(director_job_id: str) -> str:
    ...
    try:
        ...
        from src.agents.models import StrategicBrief
...
def weaver_task(tactician_job_id: str) -> str:
    ...
    try:
        ...
        from src.agents.models import ChapterBlueprint
...
@task(retries=2, retry_delay_seconds=exponential_backoff(backoff_factor=2))
def canonist_task(chapter_text: str, chapter_blueprint: dict) -> str:
    """Execute Canonist agent for final validation (automated).
    ...
    Returns:
        str: Validated chapter text
    """
    logger = get_run_logger()

    logger.info("Starting Canonist validation task")

    try:
        # Execute Canonist agent
        canonist = CanonistAgent()
        validated_text = canonist.execute(chapter_text, chapter_blueprint)

        logger.info("Canonist validation complete")
        if isinstance(validated_text, dict):
            return str(validated_text.get("validated_text", chapter_text))
        return str(validated_text)
...
async def initial_generation_flow(chapter_seed: str, active_characters: Optional[list[str]] = None) -> str:
...
def finalize_generation_flow(tactician_job_id: str) -> str:
    ...
    Returns:
        str: Final validated chapter text
    """
    logger = get_run_logger()
    ...
    # Execute Canonist task for final validation
    final_text = canonist_task(chapter_text, tactician_output.output_payload)

    logger.info("Chapter generation complete!")
    logger.info(f"Final chapter: {len(str(final_text).split())} words")
...
def approve_job_and_continue(job_id: str) -> str:
    ...
    if job.agent == "Director":
        ...
        return finalize_generation_flow(job_id)
    else:
...
if __name__ == "__main__":
    ...
    print(f"Generated chapter: {result[:200]}...")
